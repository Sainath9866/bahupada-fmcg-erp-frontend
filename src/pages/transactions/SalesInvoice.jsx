...
import { Plus, Search, Filter, Edit, Trash2, Eye, FileText, DollarSign, Calendar, User, Package, Printer } from 'lucide-react';
...
const SalesInvoice = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingInvoice) {
        await api.put(`/sales-invoices/${editingInvoice.id}`, formData);
      } else {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2">
                    <DollarSign className="w-4 h-4 text-blue-500" />
                    <span className="text-sm text-gray-600">Status: {selectedInvoice.payment_status}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-gray-400" />
                    <span className="text-sm text-gray-600">Terms: {selectedInvoice.payment_terms}</span>
                  </div>
                </div>
              </div>
