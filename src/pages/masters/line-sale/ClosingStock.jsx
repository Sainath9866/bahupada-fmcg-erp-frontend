...
import { Plus, Search, Filter, Edit, Trash2, Eye, Package, DollarSign, Calendar, Warehouse, TrendingDown } from 'lucide-react';
...
const ClosingStock = () => {
  ...
  const fetchClosingStocks = async () => {
    try {
      setLoading(true);
      const response = await api.get('/line-sale-closing-stocks');
      setClosingStocks(response.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingStock) {
        await api.put(`/line-sale-closing-stocks/${editingStock.id}`, formData);
      } else {
        await api.post('/line-sale-closing-stocks', formData);
      }
      setShowModal(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this closing stock?')) {
      try {
        await api.delete(`/line-sale-closing-stocks/${id}`);
        fetchClosingStocks();
      } catch (error) {
  ...
  const filteredStocks = closingStocks.filter(stock => {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Line Sale Closing Stock</h1>
        <button
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-4 text-center text-gray-500">No closing stocks found</td>
                </tr>
              ) : (
                    ...
                    <tr key={stock.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-900">{stock.closing_quantity}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <DollarSign className="w-4 h-4 text-purple-500" />
                          <span className="text-sm text-gray-900">₹{stock.closing_rate}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <DollarSign className="w-4 h-4 text-green-500" />
                          <span className="text-sm text-gray-900">₹{stock.closing_value}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingStock ? 'Edit Closing Stock' : 'Add New Closing Stock'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Closing Quantity *</label>
                  <input
                    type="number"
                    step="0.01"
                    required
                    value={formData.closing_quantity}
                    onChange={(e) => setFormData({...formData, closing_quantity: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    required
                    value={formData.closing_rate}
                    onChange={(e) => setFormData({...formData, closing_rate: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Closing Value</label>
                  <input
                    type="number"
                    step="0.01"
                    value={formData.closing_value}
                    onChange={(e) => setFormData({...formData, closing_value: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Closing Stock Details</h2>
              <button
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <Package className="w-4 h-4 text-green-500" />
                      <span className="text-sm text-gray-600">Quantity: {selectedStock.closing_quantity}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <DollarSign className="w-4 h-4 text-purple-500" />
                      <span className="text-sm text-gray-600">Rate: ₹{selectedStock.closing_rate}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <DollarSign className="w-4 h-4 text-green-500" />
                      <span className="text-sm text-gray-600">Value: ₹{selectedStock.closing_value}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Package className="w-4 h-4 text-blue-500" />
