...
export default function DebitNoteJournal() {
  ...
  const fetchJournals = async () => {
    setLoading(true);
    try {
      const response = await api.get('/journals?type=Debit Note');
  ...
  const handleEdit = (journal) => {
    ...
    setFormData({
      ...
      invoice_reference: journal.invoice_reference || '',
  ...
  const filteredJournals = journals.filter(journal => {
    const matchesSearch = journal.voucher_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         (journal.invoice_reference && journal.invoice_reference.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Debit Note Journal</h1>
          <p className="text-gray-600">Manage debit note journal entries</p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by voucher, narration, or invoice..."
                value={searchTerm}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Invoice Ref</th>
              ...
              {loading ? (
                ...
                filteredJournals.map((journal) => (
                  <tr key={journal.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {vendors.find(v => v.id === journal.vendor)?.name || journal.vendor || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {journal.invoice_reference || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-800">
                {editingJournal ? 'Edit Debit Note' : 'Add Debit Note'}
              </h2>
              <button
                onClick={() => {
                  setShowModal(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="3"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  placeholder="Enter narration for this debit note..."
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {editingJournal ? 'Update' : 'Create'} Debit Note
                </button>
              </div>
            </form>
          </div>
