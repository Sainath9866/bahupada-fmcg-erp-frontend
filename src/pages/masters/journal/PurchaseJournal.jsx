...
export default function PurchaseJournal() {
  ...
  const fetchJournals = async () => {
    setLoading(true);
    try {
      const response = await api.get('/journals?type=Purchase');
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Purchase Journal</h1>
          <p className="text-gray-600">Manage purchase journal entries</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-800">
                {editingJournal ? 'Edit Purchase Journal' : 'Add Purchase Journal'}
              </h2>
              <button
                onClick={() => {
                  setShowModal(false);
