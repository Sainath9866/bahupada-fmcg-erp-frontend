...
import { Plus, Search, Filter, Edit, Trash2, Eye, RotateCcw, DollarSign, Calendar, User, Package, CheckCircle } from 'lucide-react';
import api from '../../../lib/api';

const ResalableReturns = () => {
  ...
  const conditions = ['Good', 'Fair', 'Damaged', 'Expired', 'Quality Issue'];
  ...
  const fetchReturns = async () => {
    try {
      setLoading(true);
      const response = await api.get('/resalable-returns');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingReturn) {
        await api.put(`/resalable-returns/${editingReturn.id}`, formData);
      } else {
        await api.post('/resalable-returns', formData);
      }
      setShowModal(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this return?')) {
      try {
        await api.delete(`/resalable-returns/${id}`);
        fetchReturns();
      } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Resalable Returns</h1>
        <button
