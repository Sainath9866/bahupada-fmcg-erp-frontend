...
import { Plus, Search, Filter, Edit, Trash2, Eye, Warehouse, MapPin, Phone, Users, Package, Building } from 'lucide-react';
...
const BranchWarehouse = () => {
  ...
  const [filterMainWarehouse, setFilterMainWarehouse] = useState('');
  ...
  const fetchMainWarehouses = async () => {
    try {
      const response = await api.get('/warehouses?type=Main');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error saving branch warehouse:', error);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this branch warehouse?')) {
      try {
        await api.delete(`/warehouses/${id}`);
        fetchWarehouses();
      } catch (error) {
        console.error('Error deleting branch warehouse:', error);
      }
    }
  };
  ...
  const filteredWarehouses = warehouses.filter(warehouse => {
    ...
    const matchesFilter = !filterMainWarehouse || warehouse.main_warehouse_id === filterMainWarehouse;
    return matchesSearch && matchesFilter;
  });

  const getMainWarehouseName = (mainWarehouseId) => {
    const mainWarehouse = mainWarehouses.find(w => w.id === mainWarehouseId);
    return mainWarehouse ? mainWarehouse.name : 'N/A';
  };

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Branch Warehouses</h1>
        <button
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search branch warehouses..."
                value={searchTerm}
          ...
          <div className="w-48">
            <select
              value={filterMainWarehouse}
              onChange={(e) => setFilterMainWarehouse(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              <option value="">All Main Warehouses</option>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Main Warehouse</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-4 text-center text-gray-500">No branch warehouses found</td>
                </tr>
              ) : (
                  ...
                  <tr key={warehouse.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-900">{getMainWarehouseName(warehouse.main_warehouse_id)}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingWarehouse ? 'Edit Branch Warehouse' : 'Add New Branch Warehouse'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Main Warehouse *</label>
                  <select
                    required
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Branch Warehouse Details</h2>
              <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Building className="w-4 h-4 text-blue-500" />
                      <span className="text-sm text-gray-600">Main: {getMainWarehouseName(selectedWarehouse.main_warehouse_id)}</span>
                    </div>
                    <div className="flex items-center gap-2">
