...
import { Plus, Search, Filter, Edit, Trash2, Eye, Receipt, Hash, Calendar, Settings } from 'lucide-react';
import api from '../../../lib/api';

const ReceiptNumbering = () => {
  const [receiptNumbering, setReceiptNumbering] = useState([]);
  ...
  const documentTypes = ['Receipt', 'Payment Receipt', 'Advance Receipt', 'Refund Receipt', 'Collection Receipt'];
  const resetFrequencies = ['Never', 'Daily', 'Monthly', 'Quarterly', 'Yearly'];
  const statuses = ['Active', 'Inactive', 'Suspended'];

  useEffect(() => {
    fetchReceiptNumbering();
  }, []);

  const fetchReceiptNumbering = async () => {
    try {
      setLoading(true);
      const response = await api.get('/receipt-numbering');
      setReceiptNumbering(response.data);
    } catch (error) {
      console.error('Error fetching receipt numbering:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingNumbering) {
        await api.put(`/receipt-numbering/${editingNumbering.id}`, formData);
      } else {
        await api.post('/receipt-numbering', formData);
      }
      setShowModal(false);
      ...
      console.error('Error saving receipt numbering:', error);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this receipt numbering?')) {
      try {
        await api.delete(`/receipt-numbering/${id}`);
        fetchReceiptNumbering();
      } catch (error) {
        console.error('Error deleting receipt numbering:', error);
      }
    }
  };
  ...
  const filteredNumbering = receiptNumbering.filter(numbering => {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Receipt Numbering</h1>
        <button
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search receipt numbering..."
                value={searchTerm}
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-4 text-center text-gray-500">No receipt numbering found</td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingNumbering ? 'Edit Receipt Numbering' : 'Add New Receipt Numbering'}
            </h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Receipt Numbering Details</h2>
              <button
