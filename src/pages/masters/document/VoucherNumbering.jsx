...
import { Plus, Search, Filter, Edit, Trash2, Eye, FileCheck, Hash, Calendar, Settings } from 'lucide-react';
import api from '../../../lib/api';

const VoucherNumbering = () => {
  const [voucherNumbering, setVoucherNumbering] = useState([]);
  ...
  const documentTypes = ['Sales Voucher', 'Purchase Voucher', 'Payment Voucher', 'Receipt Voucher', 'Journal Voucher', 'Contra Voucher', 'Adjustment Voucher'];
  const resetFrequencies = ['Never', 'Daily', 'Monthly', 'Quarterly', 'Yearly'];
  const statuses = ['Active', 'Inactive', 'Suspended'];

  useEffect(() => {
    fetchVoucherNumbering();
  }, []);

  const fetchVoucherNumbering = async () => {
    try {
      setLoading(true);
      const response = await api.get('/voucher-numbering');
      setVoucherNumbering(response.data);
    } catch (error) {
      console.error('Error fetching voucher numbering:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingNumbering) {
        await api.put(`/voucher-numbering/${editingNumbering.id}`, formData);
      } else {
        await api.post('/voucher-numbering', formData);
      }
      setShowModal(false);
      ...
      console.error('Error saving voucher numbering:', error);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this voucher numbering?')) {
      try {
        await api.delete(`/voucher-numbering/${id}`);
        fetchVoucherNumbering();
      } catch (error) {
        console.error('Error deleting voucher numbering:', error);
      }
    }
  };
  ...
  const filteredNumbering = voucherNumbering.filter(numbering => {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Voucher Numbering</h1>
        <button
          onClick={() => setShowModal(true)}
          className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2"
        >
          <Plus className="w-4 h-4" />
          Add Voucher Numbering
        </button>
      </div>
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search voucher numbering..."
                value={searchTerm}
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-4 text-center text-gray-500">No voucher numbering found</td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editingNumbering ? 'Edit Voucher Numbering' : 'Add New Voucher Numbering'}
            </h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Voucher Numbering Details</h2>
              <button
