...
import { Plus, Search, Edit, Trash2, Filter, Building2, MapPin, Phone, CreditCard, AlertTriangle, Users } from 'lucide-react';
...
export default function Distributors() {
  ...
  const fetchDistributors = async () => {
    setLoading(true);
    try {
      const response = await api.get('/customers?type=distributor');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDistributor) {
        await api.put(`/customers/${editingDistributor.id}`, data);
      } else {
  ...
  const handleEdit = (distributor) => {
    ...
    setFormData({
      ...
      salesman: distributor.salesman || '',
      credit_limit: distributor.credit_limit || '',
      payment_terms: distributor.payment_terms || '',
      price_list: distributor.price_list || '',
      credit_risk: distributor.credit_risk || 'low',
      distributor_type: distributor.distributor_type || 'regional',
  ...
  const filteredDistributors = distributors.filter(distributor => {
    const route = routes.find(r => r.id === distributor.route);
    const salesman = salesmen.find(s => s.id === distributor.salesman);
    ...
    const matchesRoute = !filters.route || distributor.route === filters.route;
    const matchesSalesman = !filters.salesman || distributor.salesman === filters.salesman;
    const matchesCreditRisk = !filters.credit_risk || distributor.credit_risk === filters.credit_risk;
    const matchesStatus = !filters.status || distributor.status === filters.status;
  ...
  const getDistributorTypeColor = (type) => {
    switch (type) {
  ...
  const getOutstandingStatus = (distributor) => {
    const outstanding = distributor.outstanding || 0;
    const creditLimit = distributor.credit_limit || 0;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Distributors</h1>
          <p className="text-gray-600">Manage distributor customers and their information</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type/Territory</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-4 text-center text-gray-500">No distributors found</td>
                </tr>
              ) : (
                filteredDistributors.map((distributor) => {
                  const route = routes.find(r => r.id === distributor.route);
                  const salesman = salesmen.find(s => s.id === distributor.salesman);
                  const outstandingStatus = getOutstandingStatus(distributor);
                    ...
                    <tr key={distributor.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          ...
                          <div className="text-sm text-gray-500">{distributor.pincode}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDistributorTypeColor(distributor.distributor_type)}`}>
                            {distributor.distributor_type}
                          </span>
                        </div>
                        <div className="text-sm text-gray-500">{distributor.territory || '-'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <CreditCard size={14} className="text-gray-400" />
                          ₹{distributor.credit_limit?.toLocaleString() || '0'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${outstandingStatus.color}`}>
                            ₹{distributor.outstanding?.toLocaleString() || '0'}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getCreditRiskColor(distributor.credit_risk)}`}>
                          {distributor.credit_risk}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Distributor Type</label>
                  <select
                    value={formData.distributor_type}
                    onChange={(e) => setFormData({...formData, distributor_type: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  >
