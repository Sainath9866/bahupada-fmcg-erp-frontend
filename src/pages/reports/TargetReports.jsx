...
import { Search, Filter, Download, Target, TrendingUp, Users, Calendar, BarChart3, PieChart, Award } from 'lucide-react';
...
const TargetReports = () => {
  ...
  const targetReports = [
    { id: 'target-achievement', name: 'Target Achievement', description: 'Employee target vs achievement' },
    { id: 'sales-performance', name: 'Sales Performance', description: 'Sales performance by employee' },
    { id: 'team-performance', name: 'Team Performance', description: 'Team-wise performance analysis' },
    { id: 'monthly-targets', name: 'Monthly Targets', description: 'Monthly target setting and tracking' },
    { id: 'quarterly-review', name: 'Quarterly Review', description: 'Quarterly performance review' },
    { id: 'incentive-calculation', name: 'Incentive Calculation', description: 'Incentive calculations based on targets' },
    { id: 'top-performers', name: 'Top Performers', description: 'Top performing employees' },
    { id: 'under-performers', name: 'Under Performers', description: 'Employees below target' }
  ];
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await api.get(`/reports/targets?${params}`);
  ...
  const handleDownload = async (reportId) => {
    try {
      ...
      const response = await api.get(`/reports/targets/${reportId}/download?${params}`, {
        responseType: 'blob'
      });
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">₹4,25,000</p>
            </div>
          </div>
        </div>
      ...
      {selectedReport && (
        <div className="mt-6 bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            {targetReports.find(r => r.id === selectedReport)?.name} Report
          </h3>
          {loading ? (
